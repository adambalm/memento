...
async function startDetailedReview(task) {
  await logTaskAction({
    ...
    outcome: { action: 'redirect-to-review' }
  });

  return {
    success: true,
    message: 'Opening detailed review',
    redirectTo: `/tasks/review?taskId=${task.id}`
  };
}
...
async function startTriage(task) {
  await logTaskAction({
    ...
    outcome: { action: 'redirect-to-triage' }
  });

  return {
    success: true,
    message: 'Opening triage interface',
    redirectTo: `/tasks/triage?taskId=${task.id}`
  };
}
  ...
  switch (task.type) {
    ...
    case 'project_revival':
      switch (action) {
        ...
        case 'explore':
          // Future: open chat. For now, just log and show message
          await logTaskAction({
            taskId: task.id,
            taskType: 'project_revival',
            action: 'explore',
          ...
          return { success: true, message: 'Exploration feature coming soon' };
        default:
